...
function hr_offers_render_meta_box($post) {
    ?>
    <div>
        <a href="<?php echo plugin_dir_url(__FILE__) . 'generate-pdf.php?post_id=' . $post->ID; ?>" target="_blank" class="button button-primary">Show PDF <span class="dashicons dashicons-media-document"></span></a>
        <button type="button" class="button button-secondary send-email-button" data-id="<?php echo $post->ID; ?>">Send Email <span class="dashicons dashicons-email"></span></button>
    </div>
